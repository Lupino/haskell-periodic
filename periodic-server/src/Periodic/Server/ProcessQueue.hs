...
type SubProcessQueue = HashMap Int Job
...
insertJob q j@Job{..} = alter q update jFuncName
  where update :: Maybe SubProcessQueue -> Maybe SubProcessQueue
        update Nothing   = Just (HM.fromList [(hash jName, j)])
        update (Just q') = Just (insert (hash jName) j q')

lookupJob :: ProcessQueue -> FuncName -> Int -> IO (Maybe Job)
lookupJob q fn jn = maybe Nothing (HM.lookup jn) <$> lookup q fn

removeJob :: ProcessQueue -> FuncName -> Int -> IO ()
removeJob q fn jn = adjust q (delete jn) fn

memberJob :: ProcessQueue -> FuncName -> Int -> IO Bool
