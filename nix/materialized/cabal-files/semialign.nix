{ system
  ...
  ({
    ...
    package = {
      specVersion = "1.10";
      identifier = { name = "semialign"; version = "1.3"; };
    ...
    components = {
      "library" = {
        depends = (((([
          ...
          ] ++ (pkgs.lib).optional (!(compiler.isGhc && (compiler.version).ge "8.2")) (hsPkgs."bifunctor-classes-compat" or (errorHandler.buildDepError "bifunctor-classes-compat"))) ++ (pkgs.lib).optionals (!(compiler.isGhc && (compiler.version).ge "8.0")) ([
    ...
    src = (pkgs.lib).mkDefault (pkgs.fetchurl {
      url = "http://hackage.haskell.org/package/semialign-1.3.tar.gz";
